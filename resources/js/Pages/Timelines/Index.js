...
export default function Timeline(props) {
    ...
    return (
        <Authenticated auth={props.auth} errors={props.errors} header="Cronogramas">
            ...
            {(auth.user.admin === 3) &&
                <Box sx={{ display: 'flex', justifyContent: 'end' }}>
                    <Button variant="contained" color="primary" LinkComponent='a' href={route('timelines.create')}>Partilhar cronograma</Button>
                </Box>
            }
            ...
            {timelines.length === 0 && <Typography padding={2} textAlign="center" variant="h6">Nenhum cronograma foi partilhado ainda.</Typography>}
                    ...
                    {timelines.map((timeline) =>
                        <Grid key={timeline.id} item sm={12} md={5}>
                            <Card sx={{ maxWidth: 345 }}>
                                ...
                                <CardActions>
                                {auth.user.admin === 3 &&
                                                <IconButton onClick={() => confirmDelete(timeline.id)} aria-label="apagar">
                                                    <Delete color="error" />
                                                </IconButton>
                                            }
                                            <IconButton LinkComponent="a" href={`/${timeline.timeline_path}`} target="_blank" aria-label="ver">
                                                <Visibility />
                                            </IconButton>
                                </CardActions>
                            </Card>
